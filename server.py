...
work_on_server = True
#server_ip = 'localhost'
server_ip = '217.28.221.126'

FPS = 100
WIDTH_ROOM, HEIGHT_ROOM = 3000, 4000
...
colours = {'0': (255, 255, 0), '1': (255, 0, 0), '2': (0, 255, 0), '3': (0, 255, 255), '4': (128, 0, 128), '5':(255,255,255), '6':(254,91,172), '7':(150,75,0), '8':(75, 0 , 255)}
...
class Player():
    ...
    # функция меняющая координаты игроков
    def update(self):
        # x coordinate
        if self.x - self.r <= 0:
            if self.speed_x >= 0:
...
foods = [Food(random.randint(0, WIDTH_ROOM),
                    random.randint(0, HEIGHT_ROOM),
                    FOOD_SIZE,
                    str(random.randint(0, 8)))
...
while server_works:
    ...
    if tick == 200:
        ...
        # дополняем список точек с едой
        new_foods = [Food(random.randint(0, WIDTH_ROOM),
                                random.randint(0, HEIGHT_ROOM),
                                FOOD_SIZE,
                                str(random.randint(0, 8)))
    ...
    for player in players:
        if player.conn != None:
            try:
                ...
                if data[0] == '!':  # пришло сообщение о готовности передачи сообщений
                    player.ready = True
                else:
    ...
    for i in range(len(players)):
        # какие точки с едой видит i игрок
        for k in range(len(foods)):
            ...
            # i видит k
            if (
                    (abs(dist_x) <= (players[i].w_vision) // 2 + foods[k].r)
                    and
                    (abs(dist_y) <= (players[i].h_vision) // 2 + foods[k].r)
            ):
                # i может съесть k
    ...
    # чистим список от съеденных точек с едой
    for m in foods:
        if m.r == 0: foods.remove(m)
    ...
    if not work_on_server:
        ...
        screen.fill('BLACK')
        for player in players: # здесь преобразовывается масштаб изображения от клиента на окно сервера
